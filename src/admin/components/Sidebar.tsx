...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, userRole }) => {
  const { t } = useLanguage();
  const trans = t.admin;
  const transManager = t.admin.manager;
  const isAdmin = userRole === 'admin';

  const handleLogout = () => {
    localStorage.removeItem('isAdminAuth');
    localStorage.removeItem('isManagerAuth');
    window.location.href = `/admin/login/${userRole}`;
  };

  const golfItems = [
    { name: transManager.nav.courses, icon: 'golf_course', path: '/admin/courses', roles: ['admin', 'manager'] },
    { name: trans.nav.teeTimeRules, icon: 'rule', path: '/admin/tee-time-rules', roles: ['admin', 'manager'] },
    { name: trans.nav.slotManagement, icon: 'schedule', path: '/admin/slots', roles: ['admin', 'manager'] },
  ...
  const renderNavGroup = (title: string, items: typeof golfItems) => {
    if (items.length === 0) return null;
    return (
      <div className="mb-6">
        <p className="px-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 px-6">{title}</p>
        <div className="flex flex-col gap-1">
          {items.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all group mx-3 ${isActive
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-surface border-r border-border-light transition-transform duration-300 lg:relative lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="py-4 flex flex-col flex-1 overflow-y-auto">
        {renderNavGroup(trans.nav.groupGolf, golfItems)}
        {renderNavGroup(trans.nav.groupAdmin, adminItems)}
